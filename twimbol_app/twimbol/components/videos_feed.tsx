...
export const VideoThumbnail = ({post_id, video_title, video_by, username, video_thumbnail, onPress}:any) =>{

  const profile_pic = `${TWIMBOL_API_CONFIG.BASE_URL}${video_by.user.profile_pic}`
  const profile_name = `${video_by.user.first_name}${video_by.user.last_name}`
  ...
  return (
    <TouchableOpacity style={styles.thumbnail_container} onPress={() => onPress(post_id?post_id:null)}>
      ...
      >
        ...
        <Image style={styles.video_thumb_play_btn} source={icons.play} tintColor={'#efefef'}/>


      </ImageBackground>
      <View style={styles.video_thumb_info}>
        <Image source={{uri: profile_pic}}  style={styles.video_thumb_info_pic}/>
      <View style={styles.video_thumb_info_det}>
      <Text style={styles.video_thumb_title} className='font-bold'>{video_title}</Text>
      ...
      <Image source={icons.three_dot} tintColor={'#010101'} style={styles.video_thumb_options} />
      </View>
